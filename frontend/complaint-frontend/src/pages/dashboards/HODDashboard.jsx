...
import HODReviewModal from "../../components/hod/HODReviewModal";
...
import { getHODStats, getPendingComplaints, hodReviewComplaint } from "../../services/api";
import { fadeUp } from "../../components/Complaint/helpers";
import { useAutoRefresh } from "../../hooks/useAutoRefresh";

export default function HODDashboard() {
  const [pending, setPending] = useState([]);
  const [stats, setStats] = useState({ pending: 0, assigned: 0, rejected: 0 });
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      setStats(statsData || { pending: 0, assigned: 0, rejected: 0 });
    } catch (err) {
  ...
  // Initial load only — shows spinner once
  useEffect(() => {
    const init = async () => {
      setLoading(true);
      await refreshData();
  ...
  // Silent background refresh every 30s — no spinner
  useAutoRefresh(refreshData, 30000);

  const handleReviewAccept = async (complaintId, note = "") => {
    setReviewBusy(true);
    try {
      await hodReviewComplaint(complaintId, "accept", note);
  ...
  const handleReviewReject = async (complaintId, note = "") => {
    setReviewBusy(true);
    try {
      await hodReviewComplaint(complaintId, "reject", note);
      setReviewTarget(null);
      await refreshData();
    } catch (err) {
  ...
  const statsCards = [
    ...
    { label: "In Progress", value: stats.assigned, icon: "✅", color: "#5b9af5", bg: "rgba(91,154,245,0.1)" },
    { label: "Rejected", value: stats.rejected, icon: "⚠️", color: "#ef4444", bg: "rgba(239,68,68,0.1)" },
  ];
  ...
  const fullName = localStorage.getItem("full_name") || localStorage.getItem("username") || "HOD";
  ...
  return (
    <>
          ...
          <div className="dash-header-row" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: "#94a3b8", fontWeight: 500, marginTop: 7 }}>
                Review incoming complaints and assign them to faculty members.
              </p>
      ...
      {/* ── HOD Review Modal (Accept / Reject) ───────────────────── */}
      {reviewTarget && (
        <HODReviewModal
          complaint={reviewTarget}
          busy={reviewBusy}
          onClose={() => !reviewBusy && setReviewTarget(null)}
